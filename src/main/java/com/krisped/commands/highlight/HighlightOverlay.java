    ...
    public Dimension render(Graphics2D graphics) {
        ...
        if (!active.isEmpty()) {
            ...
            for (ActiveHighlight h : active) {
                ...
                } else if (h.getTargetType() == ActiveHighlight.TargetType.NPC_SPAWN) { // new highlight over last NPC spawn location
                    try {
                        if(tokenService!=null){
                            net.runelite.api.coords.WorldPoint wp = tokenService.getLastNpcSpawnPoint();
                            if(wp!=null && wp.getPlane()==client.getPlane()){
                                LocalPoint lp = LocalPoint.fromWorld(client, wp);
                                if(lp!=null){
                                    Polygon poly = Perspective.getCanvasTilePoly(client, lp);
        ...
        if (!texts.isEmpty()) {
            ...
            for (KPWebhookPlugin.ActiveOverheadText aot : texts) {
                ...
                } else if (aot.getTargetType()== KPWebhookPlugin.ActiveOverheadText.TargetType.NPC_SPAWN) { // new overhead text at NPC spawn
                    try {
                        if(tokenService!=null){
                            net.runelite.api.coords.WorldPoint wp = tokenService.getLastNpcSpawnPoint();
                            if(wp!=null && wp.getPlane()==client.getPlane()){
        ...
        if (!images.isEmpty()) {
            ...
            for (ActiveOverheadImage oi : images) {
                ...
                } else if (oi.getTargetType()== ActiveOverheadImage.TargetType.NPC_SPAWN) { // new overhead image at NPC spawn
                    try {
                        if(tokenService!=null){
                            net.runelite.api.coords.WorldPoint wp = tokenService.getLastNpcSpawnPoint();
                            if(wp!=null && wp.getPlane()==client.getPlane()){
