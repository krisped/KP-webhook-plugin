...
public class KPWebhookPlugin extends Plugin {
    ...
    // === New: Animation & Graphic change handling ===
    private boolean matchesAnimation(KPWebhookPreset.AnimationConfig cfg, int animId){ // allow negative ids (e.g. -1 idle)
        if(cfg==null) return true; if(cfg.getAnimationId()!=null) return cfg.getAnimationId()==animId; if(cfg.getAnimationIds()!=null && !cfg.getAnimationIds().isEmpty()) return cfg.getAnimationIds().contains(animId); return false; }
    private boolean matchesGraphic(KPWebhookPreset.GraphicConfig cfg, int graphicId){ // allow negative ids (e.g. -1 none)
        if(cfg==null) return true; if(cfg.getGraphicId()!=null) return cfg.getGraphicId()==graphicId; if(cfg.getGraphicIds()!=null && !cfg.getGraphicIds().isEmpty()) return cfg.getGraphicIds().contains(graphicId); return false; }

    // Projectile matching
    private boolean matchesProjectile(KPWebhookPreset.ProjectileConfig cfg, int projId){ if(cfg==null) return true; if(cfg.getProjectileId()!=null) return cfg.getProjectileId()==projId; if(cfg.getProjectileIds()!=null && !cfg.getProjectileIds().isEmpty()) return cfg.getProjectileIds().contains(projId); return false; }
    private int projectileIdentity(Projectile p){ // simplified: rely on object identity (per tick) instead of coordinate fields (API variation safety)
        return p==null?0:System.identityHashCode(p);
    }

    @Subscribe public void onAnimationChanged(AnimationChanged ev){ if(ev==null) return; Actor a=ev.getActor(); if(a==null) return; int animId; try { animId = a.getAnimation(); } catch(Exception e){ return; } // no longer early-return on negative (idle)
        boolean self=false, target=false; try { self = a==client.getLocalPlayer(); } catch(Exception ignored){} target = (a==currentTarget); if(debugWindow!=null && debugWindow.isVisible()){ try { debugWindow.logAnimation(a, self, target, animId); } catch(Exception ignored){} }
        for(KPWebhookPreset r: rules){ if(r==null||!r.isActive()) continue; KPWebhookPreset.TriggerType tt=r.getTriggerType(); if(tt== KPWebhookPreset.TriggerType.ANIMATION_SELF){ if(self && matchesAnimation(r.getAnimationConfig(), animId)) executeRule(r,null,-1,null,null); }
            else if(tt== KPWebhookPreset.TriggerType.ANIMATION_TARGET){ if(target && matchesAnimation(r.getAnimationConfig(), animId)) executeRule(r,null,-1,null,null); }
            else if(tt== KPWebhookPreset.TriggerType.ANIMATION_ANY){ boolean allowActor = self || target; KPWebhookPreset.AnimationConfig cfg=r.getAnimationConfig(); boolean match = matchesAnimation(cfg, animId); if(cfg==null || ((cfg.getAnimationId()==null) && (cfg.getAnimationIds()==null || cfg.getAnimationIds().isEmpty()))){ if(allowActor) executeRule(r,null,-1,null,null); } else if(match) executeRule(r,null,-1,null,null); }
        }
    }
    @Subscribe public void onGraphicChanged(GraphicChanged ev){ if(ev==null) return; Actor a=ev.getActor(); if(a==null) return; int graphicId; try { graphicId = a.getGraphic(); } catch(Exception e){ return; } // allow negative (none)
        boolean self=false, target=false; try { self = a==client.getLocalPlayer(); } catch(Exception ignored){} target = (a==currentTarget); if(debugWindow!=null && debugWindow.isVisible()){ try { debugWindow.logGraphic(a, self, target, graphicId); } catch(Exception ignored){} }
        for(KPWebhookPreset r: rules){ if(r==null||!r.isActive()) continue; KPWebhookPreset.TriggerType tt=r.getTriggerType(); if(tt== KPWebhookPreset.TriggerType.GRAPHIC_SELF){ if(self && matchesGraphic(r.getGraphicConfig(), graphicId)) executeRule(r,null,-1,null,null); }
            else if(tt== KPWebhookPreset.TriggerType.GRAPHIC_TARGET){ if(target && matchesGraphic(r.getGraphicConfig(), graphicId)) executeRule(r,null,-1,null,null); }
            else if(tt== KPWebhookPreset.TriggerType.GRAPHIC_ANY){ boolean allowActor = self || target; KPWebhookPreset.GraphicConfig cfg=r.getGraphicConfig(); boolean match = matchesGraphic(cfg, graphicId); if(cfg==null || ((cfg.getGraphicId()==null) && (cfg.getGraphicIds()==null || cfg.getGraphicIds().isEmpty()))){ if(allowActor) executeRule(r,null,-1,null,null); } else if(match) executeRule(r,null,-1,null,null); }
        }
    }

    @Subscribe public void onProjectileMoved(ProjectileMoved ev){ if(ev==null) return; Projectile p=ev.getProjectile(); if(p==null) return; int id=p.getId(); int ident=projectileIdentity(p);
        if(!seenProjectileIdentities.add(ident)) return; // first tick we observe it
        Actor tgt=null; try { tgt=p.getInteracting(); } catch(Exception ignored){}
        boolean targetIsLocal=false; boolean targetIsCurrentTarget=false; try { targetIsLocal = tgt==client.getLocalPlayer(); } catch(Exception ignored){} targetIsCurrentTarget = (tgt==currentTarget);
        // Use starting coords (x1,y1) if available (may be 0); fallback to current x,y. Convert local units to tiles by /128.
        boolean sourceIsLocal=false; try { Player local=client.getLocalPlayer(); if(local!=null){ LocalPoint lp=local.getLocalLocation(); if(lp!=null){ int startX = (int)p.getX1(); int startY = (int)p.getY1(); if(startX==0 && startY==0){ startX = (int)p.getX(); startY = (int)p.getY(); } int projTileX = startX/128; int projTileY = startY/128; int localTileX = lp.getX()/128; int localTileY = lp.getY()/128; if(Math.abs(projTileX-localTileX)<=1 && Math.abs(projTileY-localTileY)<=1) sourceIsLocal=true; } } } catch(Exception ignored){}
        boolean classifySelf = targetIsLocal || sourceIsLocal; boolean classifyTarget = !classifySelf && targetIsCurrentTarget; boolean classifyAny = !classifySelf && !classifyTarget;
        if(debugWindow!=null && debugWindow.isVisible()){
            try { String trig = classifySelf?"PROJECTILE_SELF": classifyTarget?"PROJECTILE_TARGET":"PROJECTILE_ANY"; Actor logActor = classifySelf && !targetIsLocal? client.getLocalPlayer(): tgt; debugWindow.logProjectile(trig, p, logActor); } catch(Exception ignored){}
        }
        for(KPWebhookPreset r: rules){ if(r==null||!r.isActive()) continue; KPWebhookPreset.TriggerType tt=r.getTriggerType(); if(tt== KPWebhookPreset.TriggerType.PROJECTILE_SELF){ if(classifySelf && matchesProjectile(r.getProjectileConfig(), id)) executeRule(r,null,-1,null,null); }
            else if(tt== KPWebhookPreset.TriggerType.PROJECTILE_TARGET){ if(classifyTarget && matchesProjectile(r.getProjectileConfig(), id)) executeRule(r,null,-1,null,null); }
            else if(tt== KPWebhookPreset.TriggerType.PROJECTILE_ANY){ if(classifyAny){ KPWebhookPreset.ProjectileConfig cfg=r.getProjectileConfig(); boolean match = matchesProjectile(cfg, id); if(cfg==null || ((cfg.getProjectileId()==null) && (cfg.getProjectileIds()==null || cfg.getProjectileIds().isEmpty()))){ executeRule(r,null,-1,null,null); } else if(match) executeRule(r,null,-1,null,null); } }
        }
    }

    // --- Restored helper methods (previously truncated) ---
    ...
    public void addOverheadText(KPWebhookPreset rule, String text, String position){
        ...
        t.text = trimmed;
        t.color = Color.WHITE; // default color
        t.blink = false;
        t.size = 16;
        t.position = pos; // "Above" | "Under" | "Center"
        t.remainingTicks = 80; // align with preset default durations
